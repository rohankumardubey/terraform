...
func TestPrepareFinalInputVariableValue(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			`Required variable not set: The variable "nullable_required" is required, but was not set.`,
		},
		{
			"nullable_required",
...

		{
			...
			cty.NilVal,
			cty.UnknownVal(cty.String),
			`Required variable not set: The variable "constrained_string_nullable_required" is required, but was not set.`,
		},
		{
			"constrained_string_nullable_required",
			cty.NullVal(cty.DynamicPseudoType),
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("%s %#v", test.varName, test.given), func(t *testing.T) {
			...
			t.Logf("test case\nvariable: %s\ngiven value: %#v", varAddr, test.given)
